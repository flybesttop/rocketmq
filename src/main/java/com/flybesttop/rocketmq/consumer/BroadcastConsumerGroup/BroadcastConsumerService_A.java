package com.flybesttop.rocketmq.consumer.BroadcastConsumerGroup;
...
/**
 * @author flybesttop
 * @description 广播消费者 A push
 */
@Component
public class BroadcastConsumerService_A {
    ...
    public void initConsumer() throws MQClientException {
        mqPushConsumer = new DefaultMQPushConsumer(RocketMQConfig.BROADCAST_CONSUMER_GROUP);
        mqPushConsumer.setNamesrvAddr(nameServerAddr);

        // 设置consumer所订阅的Topic和Tag，*代表全部的Tag
        mqPushConsumer.subscribe(RocketMQConfig.BROADCAST_TOPIC, "*");
        // 设置消费方式
        ...
        mqPushConsumer.registerMessageListener((MessageListenerConcurrently) (list, context) -> {
            try {
                System.out.printf("%s Receive New Messages: %s %s %n", Thread.currentThread().getName(), "BroadcastConsumerService_A", new String(list.get(0).getBody()));
...
//                System.out.println("topic=" + topic + ", tags=" + tags + ", keys=" + keys + ", msg=" + body);
        ...
        System.out.println("BroadcastConsumer_A start ...");
    }
